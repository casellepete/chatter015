  ...
  _createClass(App, null, [{
    ...
    value: function init() {
      ...
      timechannel.join().receive("error", function () {
        ...
        return console.log("Connected to timechannel!");
      });

      timechannel.on("time:update", function (msg) {
        return $("#servertime").html(msg.tz + " " + msg.hours + ":" + msg.mins + ":" + msg.secs);
      });

      var channel = socket.chan("rooms:lobby", {});

      channel.join().receive("error", function () {
      ...
      channel.on("new:message", function (msg) {
        return _this.appendMessage(msg);
      });
